...
class RestaurantsRouter extends model_router_1.ModelRouter {
    constructor() {
        ...
        this.findMenu = (req, resp, next) => {
            restaurants_model_1.Restaurant.findById(req.params.id, "+menu").then(rest => {
                if (!rest) {
                    throw new restify_errors_1.NotFoundError('Restaurante não existe');
                }
                else {
                    resp.json(rest.menu);
                    return next();
    ...
    applyRoutes(application) {
        ...
        application.put(`${this.basePath}/:id/menu`, [(0, authz_handler_1.authorize)("admin"), this.validateId, this.replaceMenu]);
    }
}
